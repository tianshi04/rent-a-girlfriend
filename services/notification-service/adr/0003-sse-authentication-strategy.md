...
## Quyết định (Decision)
1. **Tuân thủ tuyệt đối Auth Offloading:** Notification Service (Golang) sẽ **KHÔNG** tự parse JWT. Istio sẽ đảm nhiệm việc verify JWT từ header `Authorization: Bearer <token>`. Nếu hợp lệ, Istio bóc tách `userId` và truyền vào header nội bộ (VD: `x-user-id`) cho Golang.
