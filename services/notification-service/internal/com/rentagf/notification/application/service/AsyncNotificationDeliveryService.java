...
/**
 * Service chịu trách nhiệm gửi tin vật lý bất đồng bộ (@Async) chạy trên Virtual Threads. Áp dụng
 * thiết kế "Hikari Connection Exhaustion Guard" bằng cách loại bỏ hoàn toàn Database Transaction
...
public class AsyncNotificationDeliveryService {
  ...
  /**
   * Gửi tin bất đồng bộ chạy trên Virtual Threads. TUYỆT ĐỐI không có @Transactional ở đây để giải
   * phóng Connection DB trong suốt thời gian block mạng.
   *
  ...
  public void sendAsync(UUID notificationId, DeliveryChannel channel) {
    log.info(
        "[DELIVERY CORE] Async sending notification: {} via channel: {} inside thread: {}",
        notificationId,
        channel,
    ...
    try {
      ...
      log.error(
          "[DELIVERY CORE] Failed to prepare attempt for notification: {}", notificationId, e);
      return;
    }

    // 2. BƯỚC 2: Ngoài Transaction - Thực thi gửi mạng I/O (Virtual Thread block chờ thoải mái,
    // HikariCP hoàn toàn rảnh)
    SendResult result;
    try {
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "No sender strategy found for channel: " + channel));

      log.info(
          "[DELIVERY CORE] Executing physical delivery via Outbound Adapter for channel: {}",
          channel);
      result = sender.send(context.getNotification());

    } catch (Exception e) {
      log.error(
          "[DELIVERY CORE] Unexpected error during adapter delivery for notification: {}",
          notificationId,
          e);
    ...
    try {
      actionResult =
          txService.handleResult(notificationId, context.getAttemptId(), result, channel);
    } catch (Exception e) {
      log.error(
          "[DELIVERY CORE] Failed to handle result in database for notification: {}",
          notificationId,
          e);
      return;
    }
    ...
    if (actionResult.isShouldFallbackToFcm()) {
      log.info(
          "[FALLBACK TRIGGER] SSE failed for notification {}. Automatically fallback to FCM Push asynchronously.",
          notificationId);
      ...
      log.info(
          "[RETRY TRIGGER] Scheduling retry for notification {} via channel {} after {}s (Failed attempt #{})",
          notificationId,
          channel,
          delay.toSeconds(),
          actionResult.getFailedCount());
  ...
  /**
   * Tính toán thời gian delay Exponential Backoff tăng dần theo đặc tả kỹ thuật: - FCM: 2s -> 4s ->
   * 8s - Email: 5s -> 15s -> 45s
   */
