 ...
 * Định kỳ quét DB và lắng nghe sự kiện Startup để khôi phục các thông báo bị kẹt ở trạng thái
 * PROCESSING (ví dụ do server đột ngột bị sập, restart hoặc mất điện).
 */
@Slf4j
@Service
...
public class NotificationSelfHealingJob {
  ...
  /**
   * Tự động khôi phục các thông báo bị kẹt ngay khi ứng dụng khởi chạy thành công (Startup
   * Recovery).
   */
  @EventListener(ApplicationReadyEvent.class)
  public void onApplicationReady() {
    log.info(
        "[SELF-HEALING] Application started. Executing startup recovery job for stuck notifications...");
    executeRecovery();
  }
  ...
  /**
   * Logic khôi phục cốt lõi: Quét các Notification ở trạng thái PROCESSING được tạo trước 5 phút mà
   * chưa hoàn tất.
   */
  private void executeRecovery() {
    try {
      ...
      List<Notification> stuckNotifications =
          notificationRepository.findAllByStatusAndCreatedAtBefore(
      ...
      log.info(
          "[SELF-HEALING] Detected {} stuck notifications created before {}!",
      ...
      for (Notification notification : stuckNotifications) {
        try {
          ...
          log.info(
              "[SELF-HEALING] Re-dispatching stuck notification {} via channel {}...",
              notification.getId(),
              channel);
          ...
          log.error(
              "[SELF-HEALING] Failed to recover stuck notification: {}", notification.getId(), ex);
        }
      }
  ...
  /**
   * Xác định kênh gửi thích hợp để phục hồi thông báo bị kẹt: 1. Ưu tiên lấy kênh của nỗ lực gửi
   * (attempt) gần nhất. 2. Nếu chưa có attempt nào, lấy kênh đầu tiên trong danh sách cấu hình của
   * policyOverrides. 3. Fallback mặc định sang FCM.
   */
  private DeliveryChannel determineDeliveryChannel(Notification notification) {
    ...
    if (!configuredChannels.isEmpty()) {
      try {
        ...
        log.warn(
            "[SELF-HEALING] Failed to parse configured channel {} for notification {}",
            configuredChannels.get(0),
            notification.getId());
      }
    }
  ...
  private List<String> extractChannels(Notification notification) {
    ...
    try {
      return new com.fasterxml.jackson.databind.ObjectMapper()
          .convertValue(
              channelsObj, new com.fasterxml.jackson.core.type.TypeReference<List<String>>() {});
    } catch (Exception e) {
      return List.of();
    }
  }
