...
/**
 * Service triển khai Use Case FetchInboxUseCase. Đảm bảo transaction read-only để tối ưu hóa hiệu
 * suất PostgreSQL.
 */
@Service
...
public class FetchInboxService implements FetchInboxUseCase {
  ...
  @Override
  public List<Notification> fetchInbox(
      UUID userId, InboxCursor cursor, int limit, boolean unreadOnly) {
    ...
    if (cursor == null) {
      ...
      return notificationRepository.findByUserId(
          userId, cursor.createdAt(), cursor.id(), limit + 1, unreadOnly);
    }
  }
}
