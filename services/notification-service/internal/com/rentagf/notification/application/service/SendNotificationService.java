...
/**
 * Triển khai SendNotificationUseCase. Chịu trách nhiệm duy nhất cho việc định tuyến thông minh
 * (Smart Routing Engine) và uỷ thác gửi tin bất đồng bộ.
 */
@Slf4j
@Service
...
public class SendNotificationService implements SendNotificationUseCase {
  ...
  /**
   * Entry point gửi thông báo động (Smart Routing Engine). Dựa vào cấu hình kênh trong
   * policyOverrides và trạng thái Online để định tuyến.
   */
  @Override
  @Transactional
  public Notification routeAndSend(Notification notification) {
    log.info(
        "Processing routing and sending for notification: {}, eventId: {}",
        notification.getId(),
        notification.getEventId());

    // 1. Idempotency Guard: Chặn xử lý trùng lặp
    notificationRepository
        .findByEventIdAndUserId(notification.getEventId(), notification.getUserId())
        .ifPresent(
            n -> {
              throw new DuplicateEventException(
                  notification.getEventId(), notification.getUserId().toString());
            });

    // 2. Lưu trạng thái PENDING ban đầu vào Database
    Notification saved = notificationRepository.save(notification);

    // 3. Trích xuất danh sách kênh cấu hình từ policyOverrides (an toàn với cả Java List & Scala
    // List)
  ...
  /**
   * Trích xuất danh sách channels an toàn từ policyOverrides, tránh lỗi ép kiểu trong trường hợp
   * Jackson deserialize JSON array thành Scala List (do classpath test có Scala).
   */
