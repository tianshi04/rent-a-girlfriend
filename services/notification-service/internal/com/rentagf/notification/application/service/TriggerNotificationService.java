...
/**
 * Triển khai TriggerNotificationUseCase. Chịu trách nhiệm duy nhất cho việc gửi thông báo trực tiếp
 * qua kênh chỉ định sẵn (không qua routing động).
 */
@Slf4j
@Service
...
public class TriggerNotificationService implements TriggerNotificationUseCase {
  ...
  public Notification triggerNotification(
    ...
    log.info(
        "Triggering manual notification for user: {}, eventId: {}, channel: {}",
        userId,
        eventId,
        channel);
    ...
    Notification notification =
        Notification.create(
            userId, eventId, type, priority, payload, Map.of("channels", List.of(channel.name())));
