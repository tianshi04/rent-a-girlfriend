...
/**
 * Helper Service chứa các phương thức thực thi cơ sở dữ liệu ngắn chạy trong Transaction mới. Thiết
 * kế này là cốt lõi của "Hikari Connection Exhaustion Guard", đảm bảo giải phóng connection DB ngay
 * lập tức trước khi luồng thực hiện các cuộc gọi mạng I/O dài (FCM, Email).
 */
@Slf4j
@Service
...
public class NotificationDeliveryTransactionService {
  ...
  /**
   * Bước 1: Transaction ngắn - Load Notification, tạo Attempt và lưu để chuyển trạng thái sang
   * PROCESSING.
   */
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = Exception.class)
  public DeliveryContext prepareAttempt(UUID notificationId, DeliveryChannel channel) {
    log.debug(
        "[TX-PREPARE] Creating attempt for notification {} via channel {}",
        notificationId,
        channel);
   ...
   */
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = Exception.class)
  public DeliveryActionResult handleResult(
      UUID notificationId, UUID attemptId, SendResult result, DeliveryChannel channel) {
    log.debug(
        "[TX-RESULT] Processing delivery result for notification {}, attempt {}",
        notificationId,
        attemptId);
    ...
    if (result.isSuccess()) {
      ...
      log.info(
          "[TX-RESULT] Notification {} marked as COMPLETED via channel {}",
          notificationId,
          channel);
    } else {
      // Lỗi gửi tin ngoại vi
      notification.markAttemptFailed(
          attemptId, result.getErrorCode(), result.getErrorMessage(), result.isRecoverable());
      log.warn(
          "[TX-RESULT] Attempt {} failed. Code: {}, Error: {}",
          attemptId,
          result.getErrorCode(),
