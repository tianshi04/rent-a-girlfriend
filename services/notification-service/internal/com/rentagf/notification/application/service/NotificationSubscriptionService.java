...
/**
 * Triển khai nghiệp vụ đăng ký nhận thông báo SSE thông qua Inbound Port. Đảm bảo Single
 * Responsibility Principle (SRP) tuyệt đối bằng cách phân tách UseCase độc lập.
 */
@Slf4j
@Service
...
public class NotificationSubscriptionService implements NotificationSubscriptionUseCase {
  ...
  /**
   * Khi Client đăng ký kết nối SSE thành công, tự động truy vấn các thông báo chưa đọc cũ từ DB và
   * chủ động đẩy xuống cho Client để đảm bảo không bị mất mát tin nhắn (Inbox Delivery).
   */
  @Override
  ...
  public void subscribe(UUID userId) {
    ...
    List<Notification> unreadNotifications =
        notificationRepository.findByUserId(userId, null, null, 100, true);
    log.info(
        "Found {} unread notifications to push via SSE for user: {}",
    ...
    for (Notification notification : unreadNotifications) {
      try {
        log.debug(
            "Pushing unread notification {} to user {} via SSE", notification.getId(), userId);
        ssePort.send(notification);
      } catch (Exception e) {
        log.error(
            "Failed to push unread notification {} to user {} via SSE during subscription",
            notification.getId(),
            userId,
            e);
      }
