...
/**
 * Aggregate Root cho Notification. Hoàn toàn thuần Java, độc lập với JPA/Hibernate. Tự bảo vệ các
 * Invariants [INV-N01], [INV-N02].
 */
public class Notification {
  ...
  public Notification(
    ...
    this.policyOverrides =
        policyOverrides != null ? new HashMap<>(policyOverrides) : new HashMap<>();
  ...
  /** Tạo một nỗ lực gửi mới. Bảo vệ [INV-N01] và [INV-N02]. */
  ...
  /**
   * Đánh dấu nỗ lực gửi thất bại. Cập nhật trạng thái Notification dựa trên tính Recoverable của
   * lỗi.
   */
  public void markAttemptFailed(
      UUID attemptId, String errorCode, String errorMessage, boolean isRecoverable) {
    DeliveryAttempt attempt = findAttempt(attemptId);
  ...
  private long countFailedAttempts() {
    return this.attempts.stream()
        .filter(
            a ->
                a.getStatus() == AttemptStatus.FAILED_RECOVERABLE
                    || a.getStatus() == AttemptStatus.FAILED_UNRECOVERABLE)
        .count();
  }
        ...
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    "Attempt with ID " + attemptId + " not found in Notification " + this.id));
  }
  ...
  public Map<String, Object> getPolicyOverrides() {
    return Collections.unmodifiableMap(policyOverrides);
  }
