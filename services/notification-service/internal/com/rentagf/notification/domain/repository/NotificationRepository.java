...
/**
 * Repository Port — ranh giới giữa Domain và Infrastructure. Domain chỉ biết interface này, không
 * biết JPA/SQL. Tham chiếu: docs/data-model.md
 */
public interface NotificationRepository {
  ...
  /**
   * Inbox query: cursor-based pagination cho userId. Tham chiếu: docs/api-contract.md §2.1,
   * ADR-0004
   *
  ...
  List<Notification> findByUserId(
      UUID userId, Instant cursor, UUID cursorId, int limit, boolean unreadOnly);

  /** Đếm số thông báo chưa đọc của user. */
  long countUnreadByUserId(UUID userId);

  /**
   * Mark read: set read_at cho 1 notification của user cụ thể. Sử dụng Optimistic Update để tối ưu
   * số database round-trips.
   *
   * @param notificationId ID thông báo
   * @param userId ID người dùng sở hữu (ngăn chặn tấn công BOLA)
   * @param readAt Thời điểm đọc
   * @return true nếu cập nhật thành công (hoặc thông báo đã được đọc từ trước - Idempotent), false
   *     nếu thông báo không tồn tại hoặc không thuộc về người dùng này.
   */
   ...
   */
  List<Notification> findAllByStatusAndCreatedAtBefore(
      com.rentagf.notification.domain.vo.enums.NotificationStatus status, Instant before);
}
