...
/**
 * Adapter hạ tầng sử dụng Redis Pub/Sub để truyền thông điệp liên-máy (Inter-Pod). Triển khai
 * PubSubPort (Outbound) và MessageListener (Inbound/Distributed).
 */
@Component
public class RedisPubSubAdapter implements PubSubPort, MessageListener {
  ...
  /**
   * Constructor Injection. Sử dụng @Lazy trên sseConnectionRegistry để tháo gỡ hoàn toàn lỗi
  ...
  public void onMessage(Message message, byte[] pattern) {
    ...
    log.debug(
        "Found {} active SSE connections on this pod for user {}. Pushing data...",
        emitters.size(),
        userId);
    ...
    for (SseEmitter emitter : emitters) {
      try {
        ...
        emitter.send(SseEmitter.event().name("notification").data(cleanBody));
      } catch (IOException e) {
        log.warn(
            "Broken pipe detected when sending SSE to user {}. Triggering error handler...",
            userId);
