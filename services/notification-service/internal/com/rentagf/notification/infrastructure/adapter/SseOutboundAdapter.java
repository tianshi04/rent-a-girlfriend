...
/**
 * Adapter hạ tầng chịu trách nhiệm gửi thông báo thời gian thực qua SSE. Thực thi hợp đồng SsePort
 * bằng cách đóng gói tin nhắn và publish lên Redis Pub/Sub.
 */
@Component
public class SseOutboundAdapter implements SsePort {
  ...
  public SendResult send(Notification notification) {
    ...
    try {
      ...
      log.info(
          "Preparing to send SSE notification {} to channel {}", notification.getId(), channel);
      ...
      log.info(
          "SSE notification {} published successfully to channel {}",
          notification.getId(),
          channel);
      ...
      log.error(
          "Unexpected error occurred while publishing SSE notification {}",
          notification.getId(),
          e);
