...
/**
 * Adapter hạ tầng gửi Email sử dụng Spring Boot Starter Mail (JavaMailSender). Kết nối SMTP Server
 * thực tế để thực hiện gửi Email.
 */
@Slf4j
@Component
public class EmailOutboundAdapter implements EmailPort {
  ...
  public SendResult send(Notification notification) {
    ...
    // Email người nhận: Thử bốc từ payload nếu có, nếu không thì fallback sang định dạng mặc định
    // userId@rentagf.com
    String toEmail =
        (String)
            notification.getPayload().getOrDefault("email", userId.toString() + "@rentagf.com");
    ...
    try {
      ...
      // Lỗi SMTP thường là recoverable (vd: nghẽn mạng SMTP, timeout), đánh dấu true để cho phép
      // retry ở phase sau
