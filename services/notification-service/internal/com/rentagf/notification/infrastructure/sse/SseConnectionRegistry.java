...
/**
 * Orchestrator điều phối vòng đời kết nối SSE và đồng bộ trạng thái phân tán. Đảm bảo Single
 * Responsibility Principle (SRP) bằng cách chỉ lo việc phối hợp các ports hạ tầng, uỷ thác hoàn
 * toàn việc lưu trữ in-memory cục bộ cho SseLocalRegistry.
 */
@Component
public class SseConnectionRegistry {
  ...
  /**
   * Đăng ký một kết nối SSE mới của User. Nếu đây là kết nối đầu tiên của User trên Pod này: - Kích
   * hoạt subscribe kênh Pub/Sub. - Ghi nhận trạng thái Online lên Redis Connection State Store.
   *
  ...
  public void register(UUID userId, SseEmitter emitter) {
    ...
    if (isFirst) {
      ...
      log.info(
          "First SSE connection for user {}. Activating subscription on channel: {} and setting Redis Online status",
          userId,
          channel);
  ...
  /**
   * Hủy đăng ký kết nối SSE của User. Nếu đây là kết nối active cuối cùng của User trên Pod này: -
   * Kích hoạt unsubscribe kênh Pub/Sub. - Ghi nhận trạng thái Offline trên Redis.
   *
   * @param userId ID của người nhận (UUID)
  ...
  public void unregister(UUID userId, SseEmitter emitter) {
    ...
    if (isLast) {
      String channel = getChannelName(userId);
      log.info(
          "Last SSE connection closed for user {}. Deactivating subscription on channel: {} and removing Redis Online status",
          userId,
          channel);
  ...
  /**
   * Nhận sự kiện Ping Heartbeat thành công từ Controller. Refresh lại TTL của trạng thái Online
   * trên Redis để tránh hết hạn.
   */
