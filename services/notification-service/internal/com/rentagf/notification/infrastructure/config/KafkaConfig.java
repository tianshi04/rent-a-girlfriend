 ...
 * thông qua {@link KafkaProperties}, sau đó override duy nhất Executor để tích hợp Java 21 Virtual
 * Threads.
 *
 * <p>{@code @ConditionalOnBean(ConsumerFactory.class)} đảm bảo bean này chỉ được khởi tạo khi
 * KafkaAutoConfiguration đang hoạt động (production / integration test với EmbeddedKafka). Các unit
 * test exclude Kafka sẽ không bị ảnh hưởng.
 */
@Slf4j
@EnableKafka
@Configuration
...
public class KafkaConfig {
   ...
   *
   * <p>Sử dụng {@link KafkaProperties} để tái sử dụng toàn bộ cấu hình đã khai báo trong {@code
   * application.yml} (ack-mode, group-id, deserializer, error-handler, v.v.). Sau đó override
   * Executor để chạy trên Java 21 Virtual Threads thay vì Platform Threads.
   *
  ...
  public ConcurrentKafkaListenerContainerFactory<String, String> kafkaListenerContainerFactory(
    ...
    log.info(
        "Kafka Consumer Container Factory initialized successfully with Java 21 Virtual Threads. "
            + "AckMode: {}",
