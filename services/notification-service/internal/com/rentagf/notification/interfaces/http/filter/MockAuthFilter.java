...
/**
 * Bộ lọc Mock Xác thực chỉ kích hoạt ở môi trường phát triển (profile local hoặc dev). Tự động chèn
 * header 'user-id' mặc định nếu không có Istio Waypoint Proxy đứng trước để bypass security.
 */
@Component
@Profile({"local", "dev"})
public class MockAuthFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (userIdHeader == null || userIdHeader.isBlank()) {
      log.debug(
          "No 'user-id' header found in request. Auto-mocking with: {}", DEFAULT_MOCK_USER_ID);

      // Bọc request để ghi đè header
      HttpServletRequest wrappedRequest =
          new MockAuthRequestWrapper(request, USER_ID_HEADER, DEFAULT_MOCK_USER_ID);
  ...
  private static class MockAuthRequestWrapper extends HttpServletRequestWrapper {
    ...
    public MockAuthRequestWrapper(
        HttpServletRequest request, String headerName, String headerValue) {
      super(request);
