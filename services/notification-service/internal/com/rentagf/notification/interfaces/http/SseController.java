...
/**
 * Controller Inbound Adapter xử lý kết nối Server-Sent Events (SSE). Duy trì kết nối thời gian thực
 * non-blocking và quản lý heartbeat định kỳ [INV-N05].
 */
@RestController
...
public class SseController {
  ...
  public SseController(
    ...
    this.heartbeatScheduler =
        Executors.newScheduledThreadPool(1, Thread.ofVirtual().name("sse-heartbeat-", 0).factory());
  }

  /**
   * Bắt tay kết nối thời gian thực SSE. Cung cấp luồng streaming một chiều từ Server xuống Client.
   *
  ...
  public SseEmitter streamNotifications(@RequestHeader("user-id") UUID userId) {
    ...
    emitter.onError(
        (ex) -> {
          log.error(
              "SSE connection error occurred for user: {}, message: {}", userId, ex.getMessage());
          cleanupTask.run();
        });
