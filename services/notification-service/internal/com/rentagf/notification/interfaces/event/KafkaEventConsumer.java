 ...
 *
 * <ol>
 *   <li>Nhận raw JSON message từ Kafka.
 *   <li>Parse JSON thô → {@link CloudEvent} (Fail-Fast nếu sai cấu trúc).
 *   <li>Translate {@link CloudEvent} → {@code List<Notification>} (Fail-Fast nếu thiếu template
 *       biến).
 *   <li>Uỷ thác routing cho {@link ProcessInboundEventUseCase} (Application layer).
 *   <li>Luôn commit offset (Acknowledge) ở finally để bảo vệ hệ thống không bị nghẽn queue.
 * </ol>
 *
 * <p>Thiết kế này giữ Application Layer hoàn toàn sạch (không biết về Kafka, JSON hay CloudEvent
 * format).
 */
@Slf4j
@Component
...
public class KafkaEventConsumer {
  ...
  /**
   * Lắng nghe các Topic sự kiện từ Kafka. Parse + translate ở tầng Adapter rồi uỷ thác routing cho
   * Inbound Port.
   *
  ...
  public void consume(String message, Acknowledgment acknowledgment) {
    ...
    try {
      ...
      log.error(
          "Fail-Fast Alert: Invalid event payload. EventId: {}, Error: {}",
          eventId,
          e.getMessage(),
          e);
    } catch (Exception e) {
