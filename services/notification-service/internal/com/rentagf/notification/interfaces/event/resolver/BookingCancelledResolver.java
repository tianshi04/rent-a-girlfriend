...
/**
 * Resolver đặc thù cho sự kiện hủy lịch (BookingCancelled). Áp dụng Local Typed Record để lấy
 * Compile-time safety cho payload.
 */
@Component
  ...
  public List<UUID> resolve(Map<String, Object> eventData, String recipientField) {
    try {
      ...
      BookingCancelledPayload payload =
          objectMapper.convertValue(eventData, BookingCancelledPayload.class);

      if (payload.actorRole() == null
          || payload.clientId() == null
          || payload.companionId() == null) {
        throw new IllegalArgumentException(
            "Missing required fields (actorRole, clientId, companionId) in BookingCancelled payload");
      }
