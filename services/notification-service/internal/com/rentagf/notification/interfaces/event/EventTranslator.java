...
/**
 * Trình biên dịch sự kiện (Translator). Dịch CloudEvent thô thành một danh sách các Domain
 * Notification. Tích hợp TemplateEngine để ráp ngôn ngữ và RecipientResolverRegistry để xác định
 * người nhận.
 */
@Component
public class EventTranslator {
  ...
  /**
   * Dịch một CloudEvent thành danh sách các Notification. Áp dụng FAIL-FAST: Nếu event không hợp
   * lệ, ném Exception ngay lập tức.
   *
  ...
  public List<Notification> translate(CloudEvent event) {
    ...
    if (eventType == null || eventId == null || data == null) {
      throw new IllegalArgumentException(
          "CloudEvent missing required metadata (type, id, or data)");
    }
    ...
    for (UUID userId : recipientIds) {
      Notification notification =
          Notification.create(
              ...
              Map.of(
                  "channels",
                  channels != null ? channels : List.of()) // Lưu kênh cấu hình vào policyOverrides
              );
