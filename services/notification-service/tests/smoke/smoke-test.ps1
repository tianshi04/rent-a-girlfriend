...
while ($retryCount -lt $maxRetries) {
    ...
    if ($response -and $response.status -eq "UP") {
        Write-Host "`n✅ SUCCESS: Notification Service started successfully!" -ForegroundColor Green
...
if (-not $started) {
    Write-Host "`n❌ FAILED: Service did not start successfully or report UP status within timeout." -ForegroundColor Red
