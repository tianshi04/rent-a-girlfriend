...
class NotificationRepositoryTest {
  ...
  void testUniqueEventIdUserConstraint_duplicateShouldFail() {
    ...
    Notification notification1 =
        Notification.create(
            ...
            NotificationType.TRANSACTIONAL,
            NotificationPriority.HIGH,
            Map.of("title", "Hello 1"),
            Map.of());
    Notification notificationDuplicate =
        Notification.create(
            userId,
            eventId,
            NotificationType.TRANSACTIONAL,
            NotificationPriority.HIGH,
            Map.of("title", "Hello Duplicate"),
            Map.of());
  ...
  void testUniqueEventIdUserConstraint_differentUserShouldSuccess() {
    ...
    Notification notification1 =
        Notification.create(
            ...
            Map.of("title", "Hello 1"),
            Map.of());
    Notification notificationDifferentUser =
        Notification.create(
            userId2,
            eventId,
            NotificationType.TRANSACTIONAL,
            NotificationPriority.HIGH,
            Map.of("title", "Hello Different User"),
            Map.of());
  ...
  void testCursorBasedPagination() {
    ...
    // Tạo 3 notifications với createdAt cách nhau rõ rệt để đảm bảo thứ tự sắp xếp deterministic
    // (n1 cũ nhất, n3 mới nhất)
    Notification n1 =
        new Notification(
            UUID.randomUUID(),
            userId,
            "evt_1_" + UUID.randomUUID(),
            NotificationType.TRANSACTIONAL,
            NotificationPriority.HIGH,
            Map.of(),
            Map.of(),
    ...
    List<Notification> page2 =
        notificationRepository.findByUserId(userId, cursor, cursorId, 2, false);
  ...
  void testCountUnreadAndMarkAsRead() {
    ...
    boolean nonExistentMark =
        notificationRepository.markAsRead(UUID.randomUUID(), userId, Instant.now());
