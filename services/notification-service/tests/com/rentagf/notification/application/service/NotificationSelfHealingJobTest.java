...
class NotificationSelfHealingJobTest {
  ...
  void testPeriodicRecovery_NoStuckNotifications_NoEventFired() {
    ...
    when(repository.findAllByStatusAndCreatedAtBefore(
            eq(NotificationStatus.PROCESSING), any(Instant.class)))
        .thenReturn(List.of());
    ...
    verify(repository, times(1))
        .findAllByStatusAndCreatedAtBefore(eq(NotificationStatus.PROCESSING), any(Instant.class));
  ...
  void testPeriodicRecovery_StuckNotificationWithNoAttempts_FallbackToPolicyChannel() {
    ...
    Notification stuckNotification =
        Notification.create(
            userId,
            "evt-stuck-1",
            ...
            Map.of("title", "Stuck"),
            Map.of("channels", List.of("EMAIL", "FCM")));
