...
class AsyncNotificationDeliveryServiceTest {
  ...
  void setUp() {
    ...
    asyncDeliveryService =
        new AsyncNotificationDeliveryService(
  ...
  void testSendAsync_Success_UpdatesStatusToCompleted() {
    ...
    when(txService.handleResult(notificationId, attemptId, sendResult, channel))
  ...
  void testSendAsync_RecoverableFailure_SchedulesRetry() {
    ...
    // FCM lần 1 -> delay 2s
    verify(retrySchedulerPort, times(1))
        .scheduleRetry(eq(notificationId), eq(channel), eq(Duration.ofSeconds(2)));
    verifyNoInteractions(eventPublisher);
  }
  ...
  void testSendAsync_SseFailure_TriggersFcmFallback() {
    ...
    Notification notification =
        Notification.create(
            ...
            NotificationPriority.HIGH,
            Map.of("title", "Hello"),
            Map.of("channels", List.of("SSE", "FCM")));
    ...
    ArgumentCaptor<NotificationReadyEvent> eventCaptor =
        ArgumentCaptor.forClass(NotificationReadyEvent.class);
