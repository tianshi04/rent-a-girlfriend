 ...
 *
 * <p>Sử dụng H2 in-memory (cấu hình trong tests/application.yml) và Mockito Beans để thay thế toàn
 * bộ hạ tầng ngoài (Redis, Kafka, SMTP, FCM).
 */
@SpringBootTest
@Tag("integration")
class AsyncDeliveryTest {

  @Autowired private SendNotificationUseCase applicationService;
  ...
  @MockitoBean private FcmPort fcmPort;

  @MockitoBean private SsePort ssePort;
  ...
  void testSuccessfulAsyncDelivery() {
    ...
    when(emailPort.send(any(Notification.class))).thenReturn(SendResult.success("msg_email_ok"));
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertEquals(1, found.getAttempts().size());
              assertEquals(
                  AttemptStatus.FAILED_UNRECOVERABLE, found.getAttempts().get(0).getStatus());
            });
  }
