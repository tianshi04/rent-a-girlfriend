 ...
 *
 * <p>Mô phỏng: Lần 1: SMTP offline -> Gửi lỗi recoverable -> DB ghi nhận FAILED_RECOVERABLE,
 * Notification giữ PROCESSING. Lên lịch retry trì hoãn (Backoff). Lần 2: SMTP online -> Gửi thành
 * công -> DB ghi nhận SUCCESS, Notification chuyển sang COMPLETED.
 */
@SpringBootTest
@Tag("integration")
class NotificationRetryIntegrationTest {

  @Autowired private TriggerNotificationUseCase triggerService;

  @Autowired private NotificationRepository notificationRepository;

  @MockitoBean private ConnectionStatePort connectionStatePort;

  @MockitoBean private PubSubPort pubSubPort;

  @MockitoBean private EmailPort emailPort;
  ...
  void testEndToEnd_RetryUntilSuccess() {
    ...
    // 1. Kích hoạt gửi thông báo (Kênh Email có initial-delay-ms là 5s, nhưng ta test in-memory cực
    // nhanh)
    // Vì class cấu hình inmemory retry có thể được test với delay nhỏ,
    // Tuy nhiên, để test chạy nhanh hơn, ta kiểm tra cấu hình trong application.yml có
    // initial-delay-ms hay không.
    ...
    Notification triggered =
        triggerService.triggerNotification(
            ...
            Map.of("title", "Test E2E Retry"),
            DeliveryChannel.EMAIL);

    assertNotNull(triggered);

    // 2. Awaitility: Đợi luồng thứ nhất chạy xong -> attempt 1 thất bại -> DB lưu
    // FAILED_RECOVERABLE
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertNotNull(found);
              // Lần đầu thất bại nhưng recoverable -> status Notification vẫn là PROCESSING để chờ
              // retry
              assertEquals(NotificationStatus.PROCESSING, found.getStatus());
              assertTrue(found.getAttempts().size() >= 1);
              assertEquals(
                  AttemptStatus.FAILED_RECOVERABLE, found.getAttempts().get(0).getStatus());
            });

    // 3. Awaitility: Đợi scheduler kích hoạt lại lần 2 (Sau 5 giây backoff của Email) và gửi thành
    // công
