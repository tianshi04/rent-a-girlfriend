...
class FetchInboxServiceTest {
  ...
  void testFetchInboxWithNullCursor() {
    ...
    Mockito.when(
            notificationRepository.findByUserId(
                eq(userId), isNull(), isNull(), eq(limit + 1), eq(false)))
        .thenReturn(List.of());
    ...
    Mockito.verify(notificationRepository)
        .findByUserId(eq(userId), isNull(), isNull(), eq(limit + 1), eq(false));
  }
  ...
  void testFetchInboxWithValidCursor() {
    ...
    Mockito.when(
            notificationRepository.findByUserId(
                eq(userId), eq(createdAt), eq(cursorId), eq(limit + 1), eq(true)))
        .thenReturn(List.of());
    ...
    assertNotNull(result);
    Mockito.verify(notificationRepository)
        .findByUserId(eq(userId), eq(createdAt), eq(cursorId), eq(limit + 1), eq(true));
  }
}
