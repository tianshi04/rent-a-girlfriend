...
class GlobalExceptionHandlerTest {
    ...
    void testHandleNotFoundException() throws Exception {
        ...
        mockMvc.perform(get("/test/not-found/" + notifId))
    ...
    void testHandleDuplicateEventException() throws Exception {
        mockMvc.perform(get("/test/duplicate"))
                ...
                .andExpect(jsonPath("$.error.message").value("Event evt_123 for user usr_456 has already been processed"))
                .andExpect(jsonPath("$.error.details").isArray());
    }

    @Test
    void testHandleAlreadyCompletedException() throws Exception {
        mockMvc.perform(get("/test/already-completed"))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code").value("NOTIFICATION_ALREADY_COMPLETED"))
                .andExpect(jsonPath("$.error.message").value("Notification notif_789 is already completed. No further attempts allowed"))
                .andExpect(jsonPath("$.error.details").isArray());
    }

    @Test
    void testHandleRetryLimitExceededException() throws Exception {
        mockMvc.perform(get("/test/retry-exceeded"))
                .andExpect(status().isUnprocessableEntity())
                .andExpect(jsonPath("$.error.code").value("RETRY_LIMIT_EXCEEDED"))
                .andExpect(jsonPath("$.error.message").value("Notification notif_789 has exceeded maximum retry attempts (3)"))
                .andExpect(jsonPath("$.error.details").isArray());
    }
