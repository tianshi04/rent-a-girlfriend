...
class NotificationControllerTest {
  ...
  @MockitoBean private FetchInboxUseCase fetchInboxUseCase;

  @MockitoBean private MarkAsReadUseCase markAsReadUseCase;

  @MockitoBean private MarkAllAsReadUseCase markAllAsReadUseCase;

  @MockitoBean private TriggerNotificationUseCase triggerNotificationUseCase;

  @MockitoBean private SendNotificationUseCase sendNotificationUseCase;
  ...
  void testFetchInboxMissingUserIdHeader_shouldReturnBadRequest() throws Exception {
    mockMvc.perform(get("/v1/notifications")).andExpect(status().isBadRequest());
  }
  ...
  void testMarkAllAsReadSuccessfully() throws Exception {
    ...
    mockMvc
        .perform(patch("/v1/notifications/read-all").header("user-id", userId.toString()))
        .andExpect(status().isOk())
