...
| `FAILED` | Lệnh gửi thất bại kèm theo `error_message`. |
...
stateDiagram-v2
    ...
    state PROCESSING {
        ...
        Attempt_PENDING --> Attempt_FAILED : Network Error / Invalid Token
        
        Attempt_FAILED --> Attempt_PENDING : Retry (If < 3 limits & Recoverable)
    }
    
    Attempt_SUCCESS --> COMPLETED : Mark Notification
    Attempt_FAILED --> FAILED : Exceeded limit OR Unrecoverable
