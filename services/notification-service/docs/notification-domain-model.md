...
| `eventId` | String | ID của Event gốc (từ Broker) để đảm bảo Idempotency (không gửi trùng). |
...
- `markAttemptSuccess(attemptId: UUID)`: Đánh dấu một nỗ lực thành công. Cập nhật `status` của Notification thành `COMPLETED`.
- `markAttemptFailed(attemptId: UUID, reason: String)`: Đánh dấu nỗ lực thất bại. Kiểm tra số lần thử.
...
| `errorMessage` | Text | (Tùy chọn) Lý do lỗi chi tiết (Ví dụ: "Timeout from Firebase", "Socket disconnected"). Phục vụ debug. |
...
- **`[INV-N03] - Tính Idempotency`**: Hệ thống cấm tạo ra 2 `Notification` có cùng `eventId` và `userId` (bảo vệ ở mức DB bằng Unique Constraint).
