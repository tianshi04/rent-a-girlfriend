...
class PresignedUrlRequest(BaseModel):
    assetType: str = Field(..., description="IMAGE or VOICE", json_schema_extra={"example": "VOICE"})
    sizeBytes: int = Field(..., description="File size in bytes", json_schema_extra={"example": 1048576})
    durationSeconds: Optional[int] = Field(None, description="Required for VOICE type", json_schema_extra={"example": 25})
    contentType: str = Field(..., description="MIME content type", json_schema_extra={"example": "audio/mp3"})
...
@router.post("/profile/me/media/presigned-urls", response_model=PresignedUrlResponse, tags=["Media Management"])
async def request_presigned_url(
    ...
    try:
        ...
        return PresignedUrlResponse(
            uploadUrl=presign_data["uploadUrl"],
            fileUrl=presign_data["fileUrl"]
        )
    except DomainError as e:
