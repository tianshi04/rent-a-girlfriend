...
from internal.domain.repository import ICompanionProfileRepository, IMediaAssetRepository
...
class MediaCommandService:
    ...
    async def register_voice_intro(
        ...
        existing_voices = await self.media_repo.find_by_companion_id_and_type(companion_id, "VOICE_INTRO")
        ...
        try:
            ...
            if actual_size != declared_size:
                ...
                if actual_size > declared_size * 1.1: # Allow minor variance
                    raise ValueError(f"Actual uploaded size {actual_size} bytes exceeds declared size {declared_size} bytes")
        except Exception:
