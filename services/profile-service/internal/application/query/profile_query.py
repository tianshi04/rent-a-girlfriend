...
from internal.domain.errors import ProfileNotFoundError, ScenarioNotFoundError, ProfileNotAvailableError
...
class ProfileQueryService:
    ...
    async def get_companion_detail(self, companion_id: str, public: bool = False) -> Dict[str, Any]:
        ...
        # Get media
        voice_intros = await self.media_repo.find_by_companion_id_and_type(companion_id, "VOICE_INTRO")
        albums = await self.media_repo.find_by_companion_id_and_type(companion_id, "ALBUM")
            ...
            "scenarios": [
                {
                    ...
                    "status": sc.status
                } for sc in scenarios if sc.status == "ACTIVE" or profile.status == "PENDING"
            ]
        }
    ...
    async def search_companions(
        ...
        for comp in companions:
            ...
            starting_price = min([s.price.amount for s in active_scenarios]) if active_scenarios else 0
            ...
            data.append({
                ...
                "city": str(comp.available_cities[0]) if comp.available_cities else "Hanoi",
