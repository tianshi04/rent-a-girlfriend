...
class DbCleanupWorker:
    ...
    the outbox table to prevent unbounded growth.
    """
    ...
    async def _run_cleanup(self):
        ...
        logger.info(f"Running DB cleanup. Outbox cutoff: {cutoff_outbox}")

        async with self.session_factory() as session:
            try:
                # Delete old outbox records that have been processed
                ...
                logger.info(f"Cleaned up {res_outbox.rowcount} outbox records.")
            except Exception as e:
