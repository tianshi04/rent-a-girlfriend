...
class CompanionProfileRepository(ICompanionProfileRepository):
    ...
    async def save(self, profile: CompanionProfile) -> None:
        model = CompanionProfileModel(
            ...
            available_cities=json.dumps([str(city) for city in profile.available_cities]),
    ...
    async def search_approved(
        ...
        query = select(CompanionProfileModel).filter(CompanionProfileModel.status == "APPROVED")
        ...
        if city:
            ...
            query = query.filter(CompanionProfileModel.available_cities.like(f'%"{city}"%'))
            
        # Joined price query
        if min_price is not None or max_price is not None:
            # Filter companion profiles that have at least one active scenario in price range
            subquery = select(ScenarioModel.companion_id).filter(ScenarioModel.status == "ACTIVE")
    ...
    def _to_domain(self, model: CompanionProfileModel) -> CompanionProfile:
        cities = [Location(city_str) for city_str in json.loads(model.available_cities or "[]")]
...
class ScenarioRepository(IScenarioRepository):
    ...
    async def count_by_companion_id(self, companion_id: str) -> int:
        stmt = select(func.count(ScenarioModel.scenario_id)).filter_by(companion_id=companion_id, status="ACTIVE")
        result = await self.session.execute(stmt)
        return result.scalar() or 0

    async def delete(self, scenario_id: str, companion_id: str) -> None:
        stmt = select(ScenarioModel).filter_by(scenario_id=scenario_id, companion_id=companion_id)
        result = await self.session.execute(stmt)
        model = result.scalars().first()
        if model:
...
class MediaAssetRepository(IMediaAssetRepository):
    ...
    async def find_by_companion_id_and_type(self, companion_id: str, asset_type: str) -> List[MediaAsset]:
        stmt = select(MediaAssetModel).filter_by(companion_id=companion_id, asset_type=asset_type)
        result = await self.session.execute(stmt)
        models = result.scalars().all()
        return [self._to_domain(m) for m in models]
