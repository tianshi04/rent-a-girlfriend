...
try:
    from gen.interaction.v1.service.interaction_service_pb2_grpc import InteractionServiceStub
    from gen.interaction.v1.messages.lock_chat_room_request_pb2 import LockChatRoomRequest
...
class gRPCInteractionAdapter(IInteractionPort):
    ...
    async def hide_review_and_lock_chat(self, booking_id: str) -> bool:
        if InteractionServiceStub is None:
            logger.error("Interaction gRPC stubs not found. Make sure protos are generated.")
            return False

        try:
            logger.info(f"Calling Interaction Service to HideReview and LockChatRoom via gRPC at {self.address} for booking {booking_id}")
            async with grpc.aio.insecure_channel(self.address) as channel:
                ...
                hide_request = HideReviewRequest(
                    ...
                    reason="Dispute resolved with client refund"
                )
                hide_response = await stub.HideReview(hide_request)
                logger.info(f"HideReview status: {hide_response.status}, message: {hide_response.message}")

                # 2. Lock chat room
                lock_request = LockChatRoomRequest(booking_id=booking_id)
                lock_response = await stub.LockChatRoom(lock_request)
                logger.info(f"LockChatRoom status: {lock_response.status}, message: {lock_response.message}")

                return hide_response.status != "FAILED" and lock_response.status != "FAILED"
    ...
    async def lock_chat_room(self, booking_id: str) -> bool:
        ...
        try:
            logger.info(f"Calling Interaction Service to LockChatRoom via gRPC at {self.address} for booking {booking_id}")
            async with grpc.aio.insecure_channel(self.address) as channel:
                stub = InteractionServiceStub(channel)
                request = LockChatRoomRequest(booking_id=booking_id)
                response = await stub.LockChatRoom(request)
                logger.info(f"LockChatRoom status: {response.status}, message: {response.message}")
