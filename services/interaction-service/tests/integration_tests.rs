...
use interaction_service::domain::value_objects::{Rating, ChatContent};
...
async fn test_chat_room_repository_integration() {
    ...
    repo.save(&chat_room).await.expect("Failed to save chat room");

    // 2. Find the ChatRoom by ID
    let found_by_id = repo.find_by_id(&room_id).await.expect("Failed to find room by ID");
    ...
    let found_by_booking = repo.find_by_booking_id(&booking_id).await.expect("Failed to find room by booking ID");
    ...
    let outbox_event: (String, String, bool) = sqlx::query_as(
        "SELECT event_type, payload, processed FROM outbox WHERE event_type = $1"
    )
    ...
    let msg1 = chat_room.send_message(&client_id, ChatContent::new("Hello companion!".to_string()))
        .expect("Failed to create chat message");
    repo.save_message(&msg1).await.expect("Failed to save chat message");

    let msg2 = chat_room.send_message(&companion_id, ChatContent::new("Hello client!".to_string()))
        .expect("Failed to create chat message");
    repo.save_message(&msg2).await.expect("Failed to save chat message");

    let messages = repo.get_messages(&room_id, 10, 0).await.expect("Failed to retrieve messages");
    ...
    repo.save(&mutable_room).await.expect("Failed to update locked chat room");

    let updated_room = repo.find_by_id(&room_id).await.expect("Failed to find room after lock").unwrap();
    assert_eq!(updated_room.status, interaction_service::domain::chat_room::ChatRoomStatus::Locked);

    // Verify lock event in outbox
    let lock_event: (String, bool) = sqlx::query_as(
        "SELECT event_type, processed FROM outbox WHERE event_type = $1"
    )
...
async fn test_review_repository_integration() {
    ...
    let found_by_id = repo.find_by_id(&review_id).await.expect("Failed to find review by ID");
    assert!(found_by_id.is_some());
    let found = found_by_id.unwrap();
    assert_eq!(found.booking_id, booking_id);
    assert_eq!(found.client_id, client_id);
    ...
    let found_by_booking = repo.find_by_booking_id(&booking_id).await.expect("Failed to find review by booking ID");
    assert!(found_by_booking.is_some());
    assert_eq!(found_by_booking.unwrap().review_id, review_id);

    // 4. Find Reviews by Companion ID
    let companion_reviews = repo.find_by_companion_id(&companion_id).await.expect("Failed to find companion reviews");
    ...
    let outbox_event: (String, bool) = sqlx::query_as(
        "SELECT event_type, processed FROM outbox WHERE event_type = $1"
    )
    ...
    repo.save(&mutable_review).await.expect("Failed to update hidden review");

    let updated_review = repo.find_by_id(&review_id).await.expect("Failed to find review after hide").unwrap();
    assert!(!updated_review.is_visible);

    // Fetch companion reviews should now return 0 visible reviews
    let companion_reviews_after_hide = repo.find_by_companion_id(&companion_id).await.expect("Failed to get companion reviews");
    assert_eq!(companion_reviews_after_hide.len(), 0);

    // Verify outbox contains ReviewHidden event
    let hide_event: (String, bool) = sqlx::query_as(
        "SELECT event_type, processed FROM outbox WHERE event_type = $1"
    )
