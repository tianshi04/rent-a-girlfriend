...
async fn test_chat_room_repository_integration() {
    ...
    let outbox_event: (String, String, String, bool) =
        sqlx::query_as("SELECT event_type, payload, booking_id, processed FROM outbox WHERE event_type = $1")
    ...
    assert_eq!(outbox_event.2, booking_id, "outbox booking_id must match the Aggregate Root ID");
    ...
    let lock_event: (String, String, bool) =
        sqlx::query_as("SELECT event_type, booking_id, processed FROM outbox WHERE event_type = $1")
    ...
    assert_eq!(lock_event.1, booking_id, "outbox booking_id must match the Aggregate Root ID");
}
...
async fn test_review_repository_integration() {
    ...
    assert_eq!(outbox_event.1, booking_id, "outbox booking_id must match the Aggregate Root ID");
